...
class ModelArguments:
    ...
    vae_config_path: Optional[str] = field(
        default=None, metadata={"help": "VAE config path"}
    )
    vae_pretrained_model_path: Optional[str] = field(
        default=None, metadata={"help": "VAE model path"}
    )
...
def main():
    ...
    def load_saved_params(file_path):
    ...
    if training_args.training_type == 'ours':
        ...
        for i in range(training_args.input_layers, len(model.transformer.h)):
            for param in model.transformer.h[i].parameters():
                param.requires_grad = True  

    elif training_args.training_type == 'codebook':
        for name, param in model.named_parameters():
            param.requires_grad = False   
        for param in model.transformer.vqvae.parameters():
            param.requires_grad = True  

    elif training_args.training_type == 'after_input_layer':
        for name, param in model.named_parameters():
            param.requires_grad = False     
        for i in range(training_args.input_layers, len(model.transformer.h)):
            for param in model.transformer.h[i].parameters():
    ...
    elif training_args.training_type == 'full':
        for name, param in model.named_parameters():
            param.requires_grad = True

    elif training_args.training_type == 'except_codebook':
        for name, param in model.named_parameters():
            param.requires_grad = True
        for param in model.transformer.vqvae.parameters():
            param.requires_grad = False  

    elif training_args.training_type == 'only_ctx_layer':
        for name, param in model.named_parameters():
            param.requires_grad = False     
        for i in range(training_args.input_layers, training_args.ctx_layers):
            for param in model.transformer.h[i].parameters():
                param.requires_grad = True  

    elif training_args.training_type == 'only_output_layer':
        for name, param in model.named_parameters():
            param.requires_grad = False     
        for i in range(training_args.ctx_layers, len(model.transformer.h)):
            for param in model.transformer.h[i].parameters():
                param.requires_grad = True  
    ...
    #     for name, param in model.named_parameters():
    #         param.requires_grad = False     
    #     for i in range(training_args.ctx_layers, training_args.ctx_layers):
    #         for param in model.transformer.h[i].parameters():
    #             param.requires_grad = True  
    ...
    if training_args.frozen_layers is not None and training_args.frozen_layers =="last":
        trainer = Trainer(
            model=model,
            args=training_args,
            train_dataset=train_dataset if training_args.do_train else None,
